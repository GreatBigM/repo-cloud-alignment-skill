#!/usr/bin/env bash
# repo-cloud-alignment skill 一键安装/更新脚本（多 agent 目标）
# 用法:
#   交互选择: curl -fsSL https://gitee.com/GreatBigM/repo-cloud-alignment-skill/raw/main/install.sh -o /tmp/install.sh && bash /tmp/install.sh
#   指定目标: curl -fsSL https://gitee.com/GreatBigM/repo-cloud-alignment-skill/raw/main/install.sh | bash -s -- --target hermes,claude,zcode,pi
#   全部目标: curl -fsSL https://gitee.com/GreatBigM/repo-cloud-alignment-skill/raw/main/install.sh | bash -s -- --all
# 等价于手动复制，不经过安全扫描。重复执行 = 更新（自动备份旧版 + 版本对比）
# 镜像: github.com/GreatBigM/repo-cloud-alignment-skill（海外备选）
set -euo pipefail

REPO_URL="https://gitee.com/GreatBigM/repo-cloud-alignment-skill.git"
SKILL_NAME="repo-cloud-alignment"
...
echo ""
echo "快速开始：对 AI 说 \"拉齐云端\" / \"对齐主干\" / \"复现某构建\" 或加载 repo-cloud-alignment skill"
